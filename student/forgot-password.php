<?php
/**
 * Student Forgot Password Page - Code Based Verification
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'request_code') {
    ...
    if (empty($email)) {
        ...
        $stmt = $conn->prepare("SELECT id, full_name, username FROM users WHERE email = ? AND role = 'student'");
        ...
        if ($user) {
            ...
            $email_sent = sendStudentPasswordResetCodeEmail($email, $user['full_name'], $code);
            
            if ($email_sent) {
                $_SESSION['student_reset_email'] = $email;
                $step = 2;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'reset_password') {
    ...
    $email = $_SESSION['student_reset_email'] ?? '';
    ...
    if (empty($code)) {
        ...
        if ($reset) {
            ...
            sendStudentPasswordResetSuccessEmail($email, $reset['email'], 'Student');
            
            // Clear session
            unset($_SESSION['student_reset_email']);
...
$page_title = 'Student Forgot Password';
?>
<!DOCTYPE html>
<html lang="en">
<head>
        ...
        <div class="card">
            ...
            <form method="POST" action="">
                ...
                <div class="form-group">
                    ...
                    <input type="email" name="email" class="form-control" placeholder="student@example.com" required value="<?php echo htmlspecialchars($email); ?>">
                </div>
            ...
            <form method="POST" action="">
                ...
                <?php if (isset($_SESSION['student_reset_email'])): ?>
                <div class="info-box">
                    <i class="fas fa-envelope"></i> Code sent to: <strong><?php echo htmlspecialchars($_SESSION['student_reset_email']); ?></strong>
                </div>
                <?php endif; ?>
